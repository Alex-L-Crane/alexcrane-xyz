...
<template>
  ...
  >
    <!-- Show thumbnail (custom or default) -->
    <template v-if="!isPlayerLoaded">
      <img
        :src="thumbnailSrc"
        alt="Video thumbnail"
...
export default defineComponent({
  ...
  props: {
    ...
    customThumbnail: { type: String, default: null },
    videoWidth: { type: String, default: '100%' },
    videoHeight: { type: String, default: '100%' }
  },
  setup(props) {
    const isPlayerLoaded = ref(false)
    const youtube = ref(null)
    const useHqFallback = ref(false)

    // Use custom thumbnail if provided, else YouTube default
    const thumbnailSrc = computed(() => {
      if (props.customThumbnail) return props.customThumbnail
      return useHqFallback.value
    ...
    function onThumbError() {
      // fallback if YouTube thumbnail missing
      if (!useHqFallback.value && !props.customThumbnail) {
        useHqFallback.value = true
      }
    }
